...
const menuList = document.getElementById("userMenuContainer");
const showParishes = document.querySelector('#showAll');
...
showParishes.addEventListener('click', event => {
    onoroff = event.target.checked;
...
function createSidebarDiv(layerID, labelLayerID, textContent, appendParent, inputClassName) {
    ...
    if (inputClassName == "viewProw") {
        ...
        input.addEventListener("change", (e) => {
            ...
            var filterArray = [];
            checkedInputs.forEach(function (elem) {
                if (elem.checked) {
                    filterArray.push(elem.id);
                }
            });
            ...
            allLayerIds.forEach((layer) => {
                if (layer != "osm" && layer != "parishes" && layer != "allgeolines") {
